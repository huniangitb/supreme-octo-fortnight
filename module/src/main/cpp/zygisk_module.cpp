...
static char* redirect_path(const char *orig_path) {
    ...
    if (strncmp(temp_path, SOURCE_BASE, src_len) == 0 && temp_path[src_len] == '/') {
        const char* suffix = temp_path + src_len;
...
static int fake_openat(int dirfd, const char *pathname, int flags, ...) {
    ...
    if (new_path) { LOGI("[Redirect] openat: %s", new_path); free(new_path); }
    g_is_hooking = false;
    return res;
}
...
static int fake_mkdirat(int dirfd, const char *pathname, mode_t mode) {
    ...
    if (new_path) { LOGI("[Redirect] mkdirat: %s", new_path); free(new_path); }
    g_is_hooking = false;
    return res;
}
...
static int fake_faccessat(int dirfd, const char *pathname, int mode, int flags) {
    ...
    if (new_path) { LOGI("[Redirect] faccessat: %s", new_path); free(new_path); }
    g_is_hooking = false;
    return res;
}
...
static int fake_fstatat(int dirfd, const char *pathname, struct stat *buf, int flags) {
    ...
    if (new_path) { LOGI("[Redirect] fstatat: %s", new_path); free(new_path); }
    g_is_hooking = false;
    return res;
}

static void install_hooks() {
    LOGI("Installing MediaProvider hooks (Anti-Leak Mode)...");
...
static void companion_handler(int client_fd) {
    ...
    if (read_len <= 0) { close(client_fd); return; }
...
public:
    ...
    void postAppSpecialize(const zygisk::AppSpecializeArgs *args) override {
        ...
        if (is_media) {
            LOGI("Media process: %s. Applying All-In-One Redirection.", process_name);
            install_hooks();
